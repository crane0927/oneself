...
- [x] T001 确认分支 001-constitution-compliance 存在，且 specs/001-constitution-compliance/spec.md、plan.md、research.md、data-model.md 存在
- [x] T002 在仓库根目录（pom.xml 所在处）执行 `mvn clean install` 建立基线；如有既有构建失败须先修复再继续
...
- [x] T003 确认根 pom.xml 中 Java 版本为 21（原则 1）；否则在 pom.xml 中设置 `<java.version>21</java.version>`
...
- [x] T004 [US1] 在根 pom.xml 中将 Spring Boot 升级至 3.5.9（parent spring-boot-starter-parent 版本）
- [x] T005 [US1] 在根 pom.xml 中将 Spring Cloud 升级至 2025.0.1、Spring Cloud Alibaba 升级至 2025.0.0.0（properties 与 dependencyManagement）
- [x] T006 [US1] 在根 dependencyManagement 中增加 Flyway，在 oneself-service/oneself-system/pom.xml 中增加 flyway 依赖；在 oneself-service/oneself-system/src/main/resources/application.yml（或 application-dev.yml）中为 PostgreSQL 增加 Flyway 配置
- [x] T007 [US1] 确保 oneself-service/oneself-system 以 PostgreSQL 为主数据源（application-dev.yml / application.yml）；若保留 MySQL，在 README 或 plan 中说明为辅助或迁移路径
- [x] T008 [US1] 执行 `mvn clean install` 并修复因版本升级导致的依赖或编译问题（gap-001、gap-002、gap-003）
...
- [x] T009 [US2] 在 specs/001-constitution-compliance/plan.md 或 README 中记录模块与宪法对应关系（common-infra-jdbc 与 infra-db、common-feature 子模块等）；在 Complexity Tracking 中记录已接受的例外
- [x] T010 [US2] 校验模块依赖方向：service-api 不依赖 infra、无循环依赖；在涉及到的 pom.xml 中修复违规（原则 11–12、依赖矩阵）
...
- [x] T011 [US3] 替换或移除 oneself-service-api/oneself-system-api/src/main/java/com/oneself/model/vo/UserVO.java：在 model/dto 中引入 UserDTO（若不存在），并更新 UserClient 及所有 Feign 调用处使用 DTO；删除或迁出 vo 包（gap-004，原则 13）
- [x] T012 [US3] 确保 oneself-service/oneself-system/src/main/java/com/oneself/model/pojo/ 下所有实体类继承 oneself-common-infra-jdbc 的 BasePojo；若不重命名，在 plan 或 quickstart 中文档化 BasePojo = BaseEntity 等价（gap-006，原则 16）
- [x] T013 [US3] 确认无 Controller 或 Feign 接口暴露 Entity 类型；确保 oneself-service/oneself-system 与 oneself-service-api 中响应仅使用 Result/DTO/VO（原则 13，FR-008）
...
- [x] T014 [US4] 确认 oneself-auth、oneself-gateway、oneself-service/oneself-system 下 src/main/resources 中配置均为 YAML；移除或迁移任何 .properties，若有例外在 plan 中说明（原则 10）
- [x] T015 [US4] 将 oneself-service/oneself-system/sql/ 重构为按版本子目录（如 v1.0.0/），将现有 postgresql.sql/mysql.sql 内容迁入版本脚本；按原则 15 为每版本添加 README 及 sql/README.md（gap-005）
- [x] T016 [P] [US4] 按原则 17 在 oneself-auth/ 下新增 docker/Dockerfile.build 与 docker/Dockerfile.run（gap-007）
- [x] T017 [P] [US4] 按原则 17 在 oneself-gateway/ 下新增 docker/Dockerfile.build 与 docker/Dockerfile.run
- [x] T018 [P] [US4] 按原则 17 在 oneself-service/oneself-system/ 下新增 docker/Dockerfile.build 与 docker/Dockerfile.run
...
- [x] T019 在仓库根目录更新 README.md：写入与宪法一致的技术栈（Java 21、Spring Boot 3.5.9、Spring Cloud 2025.0.1、Alibaba 2025.0.0.0、PostgreSQL 主库、Flyway）及模块概要
- [x] T020 执行 quickstart.md 验证：运行 `mvn clean install` 并完成 specs/001-constitution-compliance/quickstart.md 中的合规检查清单（目标约 15 分钟）
- [x] T021 若有未收敛例外，在 specs/001-constitution-compliance/plan.md 的 Complexity Tracking 表中更新：例外项、原因与计划收敛日期
